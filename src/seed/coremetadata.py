...
customer = Table(
        ...
        Column('age', Integer, CheckConstraint('age>0'), CheckConstraint('age<100')),
        Column('married', Boolean, default=False),
        Column('height', Float),
        Column('weight', Float),
        Column('last_updated', DateTime, default=datetime.now, onupdate=datetime.now),
...
address = Table(
        'address', metadata,
        Column('id', UUID(as_uuid=True), primary_key=True, default=uuid4),
        Column('customer_id', UUID(as_uuid=True), ForeignKey("customer.id", onupdate='CASCADE', ondelete='CASCADE'), nullable=False),
